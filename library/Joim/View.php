...
class View {
	...
	public function show($data=[]) {
		...
		if ($this->useMain) {
			$section = (Session::isLoggedIn())? 'main' : 'public';
			...
			$jsBuffer = implode("\n", $jsBuffer);
			$this->parse($header, ['js' => $jsBuffer]);
			// Parse HTMLs on the master template
			$this->parse($master, ['content' => $template, 'head' => $header, 'foot' => $footer]);
	...
	public function appendInto(&$view, $name) {
		$template = self::get($this->page, $this->section);
		$this->parse($template, $this->data);
		$view->addData([$name => $template]);
	}
	 ...
	 * @param array $data
	 */
	protected function parse(&$html, $data=[]) {
	 ...
	 * @param array $data
	 */
	protected static function parseProcess($html, $data=[]) {
	 ...
	 * @param array $data
	 */
	public static function parseInto($html, $data=[]) {
